...
const envMap = new Map([['development', 'dev'], ['test', 'test'], ['production', 'prod']]);
...
class Config {
  constructor(configPath) {
    ...
    this[CONFIG] = {}; // parse configuration files
  ...
  [PARSE]() {
    const currentEnv = this.env; // 读取名称不包含 '.' 的配置文件

    fs.readdirSync(this[CONFIG_PATH]).filter(file => !~path.basename(file, '.js').indexOf('.')).forEach(file => {
    ...
    fs.readdirSync(this[CONFIG_PATH]).filter(file => ~file.indexOf(`.${currentEnv}.js`)).forEach(file => {
      const currentConfig = require(path.join(this[CONFIG_PATH], file));
  ...
  set(name, value = null) {
    if (typeof name === 'string') {
      ...
      const nameValue = this[SET_VALUE](names, value); // Merge configuration attributes
  ...
  get(name = null, def = null) {
    let value = this[CONFIG]; // Gets all the configuration when name is empty

...
class Worker {
  ...
  disconnect(refork = true) {
    ...
    if (refork) {
      ...
      // 给主进程发送通知：工作进程即将关闭
      worker.send(WORKER_DISCONNECT); // The daze-did-fork, which is sent back by the main process
  ...
  run() {
    ...
    if (!this.options.sticky) {
      ...
      process.on('message', (message, connection) => {
        ...
        debug('WORKER #%d  got conn from %s', process.id, connection.remoteAddress); // emulate a connection event on the server by emitting the
        // event with the connection master sent to us
        // 通过使用发送给我们的连接主发送事件，模拟服务器上的连接事件

        this.server.emit('connection', connection); // resume as we already catched the conn
        // 从一个暂停的套接字开始读数据

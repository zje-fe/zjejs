...
class Container {
  ...
  isShared(abstract) {
    return this.instances.has(abstract) || this.binds.get(abstract) && Reflect.has(this.binds.get(abstract), 'shared') && this.binds.get(abstract).shared === true;
  }
  /**
  ...
  [BIND](abstract, concrete = null, shared = false) {
    if (!abstract || !concrete) return; // console.log(concrete)
   ...
   * @param {boolean} newInstance forced instantiation
   */


  make(abstract, args = [], newInstance = false) {
    const shared = this.isShared(abstract);
    let obj = null; // returns directly if an object instance already exists in the container
    // instance shared

    if (this.instances.has(abstract) && shared && !newInstance) {
